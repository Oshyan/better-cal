...
2. **Email-to-calendar that actually works.** Forward any event email to your calendar's address and a tiered pipeline turns it into an event: real iMIP invites, schema.org markup, embedded add-to-calendar links, and finally a guarded LLM read of the message. RSVP from the calendar and it sends a genuine iMIP reply from your own address, so organizers see a normal response. Invitations land on their own calendar with organizer and attendee details preserved.
...
4. **Natural language everywhere, deterministic first.** Quick-add parses "Dinner with Alex Fri 7pm at Zuni" instantly with a rule-based parser, escalating to an LLM only when needed and always guarded against LLM mistakes (past dates, dropped people, mangled titles). Prompt filters let you describe a filter in plain English and have it evaluated over events. A trainable ranker learns from your thumbs and attendance to surface what matters in busy subscribed feeds.

5. **Leaving Google without breaking your life.** Every "Add to Google Calendar" link on the web can land in Better-Cal instead: a Chrome extension redirects them, the /add deep link and paste-a-GCal-URL quick-add parse them, and the PWA registers as the webcal/ICS handler and share target on mobile (the thing Google never fixed on Android). One command imports your entire Google export, and subscribed feeds can be adopted as local calendars.
