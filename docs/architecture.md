...
- Config via environment (set in `.env` file at app root on server, loaded by config.php): `BETTERCAL_DB_DSN`, `BETTERCAL_DB_USER`, `BETTERCAL_DB_PASS`, `BETTERCAL_GEMINI_API_KEY`, `BETTERCAL_BASE_URL`, `BETTERCAL_SESSION_SECRET`, `BETTERCAL_VAPID_PUBLIC`, `BETTERCAL_VAPID_PRIVATE`, `BETTERCAL_VAPID_SUBJECT` (Web Push reminders; generate keys once with `php server/bin/vapid.php --generate`), `BETTERCAL_SMTP_HOST`, `BETTERCAL_SMTP_PORT` (default 587, STARTTLS), `BETTERCAL_SMTP_USER`, `BETTERCAL_SMTP_PASS`, `BETTERCAL_SMTP_FROM` (optional SMTP relay for reminder emails via phpmailer; email delivery stays off while host/from are unset), `BETTERCAL_MAPTILER_KEY` (optional MapTiler raster tile key for the event detail mini-map; exposed to the client via `GET /api/v1/config`, OSM tiles when unset).
