...
- Every event has a relationship to you, derived from what its calendar is (mine / opportunities / context) and your own marks: **planned** (I'm doing it; the default on my calendars), **maybe** (tentative, drawn as an outline), **available** (an opportunity I haven't picked; drawn quietly), **context** (information such as sunsets or tides; quieter still, under the day's plans), or hidden. Feed events are available until picked. This is your side only; it never RSVPs or notifies anyone. Quick filters (toolbar, hotkeys p/m/a/x, palette) show any combination in every view, so "what am I actually doing" is one keystroke.
