...
Keeping it removed: `nginx.conf` is a dpkg conffile of CloudPanel's own `nginx-common`, and `clp-update` (which pipes CloudPanel's remote installer to bash and installs through apt) could put the line back. `/usr/local/sbin/cloudpanel-nginx-realip-verify --apply` removes it again with backup, `nginx -t` (restoring the original on failure), a graceful reload, a syslog line, `/var/log/nginx-realip-verify.log`, and an email to the owner. It runs after every apt/dpkg operation (`/etc/apt/apt.conf.d/99-nginx-realip-verify`, a `DPkg::Post-Invoke` hook) and weekly as a second step of the existing `cloudpanel-webmail-policy-verify` service (drop-in `nginx-realip.conf`). Without `--apply` it only reports. A snippet in `conf.d` could not have done this: `set_real_ip_from` is additive, and `conf.d` is not even included on this box.
